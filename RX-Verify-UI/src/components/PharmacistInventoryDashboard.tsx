...
const PharmacistInventoryDashboard: React.FC = () => {
  ...
  const [viewingReceipt, setViewingReceipt] = useState<ReceiptEventType | null>(null);
  ...
  return (
    <div className="bg-[#0a0e1a] text-white font-display h-screen flex overflow-hidden">
                ...
                <div className="space-y-3">
                  {receiptsLog.map((receipt, idx) => (
                    ...
                    >
                      ...
                      {/* Divider + batch UUID + View Button */}
                      <div className="mt-3 pt-3 border-t border-gray-700/60 flex items-center justify-between">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-white rounded-lg text-xs font-bold transition-colors"
                        >
      ...
      {viewingReceipt && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-[#151923] border border-gray-700 rounded-2xl p-0 max-w-lg w-full shadow-2xl overflow-hidden">
            <div className="h-2 w-full bg-[#00C853]" />
            <div className="p-6">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                  <FileText className="w-5 h-5 text-[#00C853]" />
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl mb-5 bg-[#00C853]/10 border border-[#00C853]/30">
                <CheckCircle className="w-5 h-5 text-[#00C853]" />
                <div>
                  <p className="font-bold text-sm text-[#00C853]">Authentic Batch Verified</p>
                  <p className="text-xs text-gray-400">Historical verification record</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3 mb-5">
                {[
                  ['Medicine', viewingReceipt.lot_medicine_name || 'Verified Medicine'],
                  ['Batch #', viewingReceipt.lot_batch_number],
                  ['Pharmacist', viewingReceipt.user_username],
                  ['Date', new Date(viewingReceipt.created_at).toLocaleString()],
                ].map(([label, value]) => (
                  <div key={label} className="bg-[#0a0e1a] border border-gray-700 rounded-lg p-3">
                    <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">{label}</p>
              ...
              <div className="bg-[#0a0e1a] border border-gray-700 rounded-lg p-3 mb-5">
                  <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Manifest ID / Lot</p>
                  <p className="text-xs font-mono text-white break-all">{viewingReceipt.lot}</p>
              </div>
